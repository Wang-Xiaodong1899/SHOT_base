...
def data_load(args): 
    ...
    dsets["source_tr"] = ImageList(tr_txt,root=s_root, transform=image_train())
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["source_te"] = ImageList(te_txt,root=s_root, transform=image_test())
    ...
    dsets["test"] = ImageList(txt_test,root=t_root, transform=image_test())
...
def cal_acc_oda(loader, netG, netB, netC):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
...
def train_source(args):
    ...
    torch.save(best_netG, osp.join(args.output_dir_src, "source_F.pt"))
...
def test_target(args):
    ...
    if args.da == 'oda':
        acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders['test'], netG, netB, netC)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='shot_ckpt')
    ...
    if args.dset=='visda':
        ...
        args.test_dset_path = args.folder + args.dset + '/' +names[args.t] +'.txt'
    args.out_file = open(osp.join(args.output_dir_src, 'log_fixcov.txt'), 'w')
    args.out_file.write(print_args(args)+'\n')
    args.out_file.flush()
    train_source(args)

    args.out_file = open(osp.join(args.output_dir_src, 'log_fixcov_test.txt'), 'w')
